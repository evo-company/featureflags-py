...
def fixture_variable():
    return graph_pb2.Variable(
        id=f.pystr(), name=f.pystr(), type=graph_pb2.Variable.STRING
    )


@pytest.fixture(name="check")
def fixture_check(variable):
    return graph_pb2.Check(
        id=f.pystr(),
...
def fixture_condition(check):
    return graph_pb2.Condition(
        id=f.pystr(), checks=[graph_pb2.Ref(Check=check.id)]
    )


@pytest.fixture(name="flag")
def fixture_flag(condition):
    return graph_pb2.Flag(
        id=f.pystr(),
        name=f.pystr(),
    ...
    with patch.object(manager._stub, "Exchange") as exchange:
        ...
        exchange.assert_called_once_with(
            service_pb2.ExchangeRequest(
                project="aginst",
                variables=[
                    service_pb2.Variable(
                        name=variable.name, type=graph_pb2.Variable.STRING
                    ),
                ],
                query=get_grpc_graph_query("aginst"),
            ),
            timeout=None,
        )
